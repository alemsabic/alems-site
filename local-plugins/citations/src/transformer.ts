...
export interface CitationsOptions {
  ...
  csl: string;
  // ale.ms/gpunkt.org customization: real rehype-citation options the upstream wrapper doesn't
  // pass through (it hand-picks a fixed set of fields rather than spreading opts). v4 relied on
  // these for hover tooltips (decoded client-side by tooltips.inline.ts / local-plugins/site-scripts).
