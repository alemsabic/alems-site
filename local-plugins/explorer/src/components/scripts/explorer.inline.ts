...
class FileTrieNode {
  ...
  get displayName() {
    ...
    // ale.ms/gpunkt.org customization: prefer shortTitle (Zotero sources), same as the
    // server-side fileTrie.ts displayName getter.
    const titleToUse = this.data?.shortTitle || nonIndexTitle;
    return titleToUse || this.fileSegmentHint || this.slugSegment || "";
  }
