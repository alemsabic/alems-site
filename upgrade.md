...
**Still to do**: Tagline/ContentHeader/Footer components, custom.scss port + reconciliation (fold in the now-orphaned `listPage.scss`/`recentNotes.scss` CSS as global `custom.scss` rules rather than plugin forks, since no core file owns them anymore), static assets, i18n string tweaks.
